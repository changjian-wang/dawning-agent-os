...
public sealed class ActionClassifierTests
{
    ...
    {
        Assert.That(() => Sut.Classify(null!), Throws.ArgumentNullException);
    }
    ...
    public void Classify_UnknownCode_Throws()
    {
        ...
        Assert.That(() => Sut.Classify(new ActionKind("unknown.thing")), Throws.ArgumentException);
    }
    ...
    public void Classify_ManuallyConstructed_StillResolvesByCode()
    {
        ...
        Assert.That(Sut.Classify(new ActionKind("memory.write")), Is.EqualTo(ActionLevel.L2));
    }
