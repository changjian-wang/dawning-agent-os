...
archived_reason: ""
supersedes: []
related: [pages/adrs/mvp-first-slice-chat-inbox-read-side.md, pages/adrs/repository-shape-product-monorepo-with-wiki.md, pages/adrs/mvp-desktop-stack-electron-aspnetcore.md, pages/adrs/backend-architecture-equinox-reference.md, pages/adrs/testing-stack-nunit-v0.md, pages/rules/plan-first-implementation.md]
...
- DI 注册策略、IUnitOfWork 形态、Mediator / Pipeline Behaviors 与架构测试约束由 [ADR-018 后端架构参考 Equinox：DDD + MediatR + Result 模式](backend-architecture-equinox-reference.md) 接管；本 ADR 不重复定义这些细节。ADR-018 取代本 ADR 中关于"V0 不使用按命名后缀自动扫描"的约束，改为按命名后缀自动注册作为默认机制。
- Api 入口只负责本地宿主、middleware 顺序和调用 `Infra.CrossCutting.IoC` 提供的集中注册扩展，不在 `Program.cs` 中堆叠大量注册细节。
...
本方案在 V0 骨架通电并完成 health endpoint、startup token、SQLite/Dapper 与架构测试后复议；或在进入 inbox item、Memory Ledger、LLM provider 第二阶段方案前复议；或在 [ADR-018](backend-architecture-equinox-reference.md) 关于后端架构参考的决策发生变更时同步复议。若执行方案中发现目录边界、进程模型、SQLite/Dapper 验证顺序或第一刀非目标需要调整，应先更新 ADR 或补充新 ADR，而不是直接开始 scaffold。
...
- [ADR-018 后端架构参考 Equinox：DDD + MediatR + Result 模式](backend-architecture-equinox-reference.md)：接管后端 DI / IUnitOfWork / Mediator / Pipeline / 架构测试细节。
