...
1. **决策可追溯（永久）**：从项目第 0 天起，每一个非显然的架构选择、每一次取代旧决策的转折，都留下 ADR + 对应 comparison。无论几个月还是几年后翻出来，都能完整复原"什么时间、基于什么信息、做了什么选择、否决了哪些备选、后来是否被推翻"——这条时间线本身就是项目最重要的资产。
...
target_user:
  - 不会熟练驱动 LLM 的人（无论是否程序员）
  - 痛点：问题问得抽象、不知道怎么把需求翻译给 LLM、用了几次没效果就放弃
  - 包含：写不清需求的程序员、不擅长 prompt 工程的高级用户——他们也是"懒得描述清楚"群体的一部分
not_target_user:
  - 已经能高效驱动 LLM 完成任务、追求极致控制粒度的高级用户（他们用 raw API 或现有工具更顺手）
in_scope_examples:
...
- 可逆且不对外可见（拼写修正、本地文件归类、给变量改名）→ agent 可自决，事后告知即可
- 不可逆或对外可见（发邮件、付款、对外发言、git push、删除文件）→ 必须 user 一键确认
...
- "帮我把这个 Python 脚本改成异步" → 自决修改本地代码，但 commit/push 须确认，✅⚠️
...
- **最小可用形态（MVP）**：尚未确定。候选方向是先聚焦一个高频生活场景（日程 / 信息整理 / 生活决策），把"长期记忆 + 抽象指令解析"做扎实，再向外扩。一旦确定，本节升级为具体描述并补 ADR。
- **最终形态**：尚未确定。允许长期开放。
- **每次决策必答的两个问题**：
...
- 协议层（MCP / A2A）在产品里的位置：内核能力，还是外挂适配？这个决策直接影响未来 framework 的边界。
- Agent 的 self-improvement / skill 演化机制是不是产品差异化能力？应不应该进入 framework？
...
- **产品 / 商业策略**：roadmap、商业模式、竞品市场分析。
- **个人随手摘录**：未经压缩、未连接到任何设计判断的二手转述。
- **教程内容**：how-to 类入门文档不进 wiki，进 README / 官方 docs。
...
- **同一概念在不同来源中名字不同**（如 skill vs tool vs capability）→ 选一个 canonical 页，其它名字在该页 §相关页面 / synonyms 注明。
- **非中英文一手资料** → 仍可收录，但 entity / concept 的摘要必须用中文。
...
- **重要动作必须显式确认；代笔必须明示**。所有不可逆或对外可见的动作（发邮件、付款、对外发言）须主人一键确认；管家替主人起草的内容，主人自己必须知道这是起草而非代发。重要性级别本身就是产品功课，写入 §4.2 持续打磨。
...
- **长期记忆是核心而非可选模块**。管家的价值 80% 来自"他了解你"，没有持续记忆的管家叫前台。记忆建模是产品差异化点，不外包给 framework，必须自研。
...
- **重要性级别如何定义？** §4.1 红线"重要动作必须显式确认"依赖一个清晰的级别定义（什么算重要、什么算可逆、什么算对外可见）。这是兑现该红线的产品功课。
- **自决 vs 请示的分界线在哪？** 什么级别的事管家可以自己做（拼写修正、归类邮件），什么必须先问主人（发邮件、订机票、做承诺）？
...
- **ADR 优先级**：§4.1 共 10 条边界判断，无须一次性补齐全部 ADR。下面 3 条是**产品哲学根基**，应作为首批 ADR 立刻起草；其余 7 条等真正被产品决策触发时再补：
  1. 管家定位（主客体边界）
  2. 选择题优先于问答题（options-over-elaboration）
  3. 长期记忆是核心而非可选模块
...
- 本文件不受 SCHEMA §7 拓扑约束（它是契约文件，规模由内容驱动）。
- 任何对 §2 yaml 锚定块、§3 收录范围、§4.1 红线 的修改都必须以 ADR 形式记录变更理由，不允许静默改动。

---

*Purpose 版本：1.2 | 最后更新：2026-04-27 | 与 SCHEMA.md 协同演化*
