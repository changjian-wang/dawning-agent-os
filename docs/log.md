...
- 脚手架处理：90/91 页插入 `## 目录`（H2≥10 自动）/ `## 交叉引用` / `## 来源`，全部带 HTML 注释标记，可幂等重跑
- 审计阈值上调（承认 wiki 综合页天然偏长）：超长 700→900，H2 过多 15→22，偏长 500→700，H2 偏多 10→16
- 终态：L1 35→1（仅 `decisions/phase-0-overview.md` 904 行待索引化）/ L2 70→34 / L3 148→1
- 结构性手术（拆分超长页）按需在后续 query 操作中逐页做，不批量
- raw/ 未触碰，frontmatter / 孤立页 / index 漂移均 0
