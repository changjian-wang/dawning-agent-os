...
> 本文档记录从零构建 Agent Framework 的所有技术决策和原因。
...
```
进程 A (Web 服务器)          进程 B (GPU 机器)          进程 C (安全沙箱)
  ┌──────────────┐          ┌──────────────┐          ┌──────────────┐
  │ 接待 Agent    │ ──msg──► │ 分析 Agent    │ ──msg──► │ 执行 Agent    │
  │ (接收用户请求) │          │ (需要 GPU)    │          │ (高权限操作)  │
  └──────────────┘          └──────────────┘          └──────────────┘
         │                         │                         │
         └─────────────────────────┴─────────────────────────┘
...
```
多 Agent 协作 ←──需要──► 长期记忆（Agent 之间通过共享记忆交换知识）
     ↑                        ↑
     │                        │
  需要分布式运行            需要远程存储
     │                        │
...
```
Dawning.Agents.Abstractions  → 接口、record、enum（零依赖包）
Dawning.Agents.Core          → 实现类 + DI 扩展方法
```
...
├── Dawning.Agents.Anthropic/          # Claude Provider
├── Dawning.Agents.OpenTelemetry/      # 可观测性（跨 Agent 追踪）
├── Dawning.Agents.Memory.Redis/       # Redis 长期记忆后端
├── Dawning.Agents.Memory.Postgres/    # PostgreSQL+pgvector 长期记忆后端
├── Dawning.Agents.Memory.Sqlite/      # SQLite 长期记忆后端（轻量/开发用）
├── Dawning.Agents.Transport.Redis/    # Redis Pub/Sub Agent 通信
├── Dawning.Agents.Transport.RabbitMQ/ # RabbitMQ Agent 通信
├── Dawning.Agents.MCP/                # Model Context Protocol
tests/
├── Dawning.Agents.Abstractions.Tests/
├── Dawning.Agents.Core.Tests/
├── ...
```
...
用户输入 "查询最近的订单并发送邮件"
    ...
    ▼
┌─────────────────┐
│ 0. 检索长期记忆   │  recall("订单 邮件") → 注入相关历史知识
└──────┬──────────┘
       │
       ▼
┌─────────────────┐
│ 1. 构建消息列表   │  system prompt + 记忆上下文 + user message + 历史消息
└──────┬──────────┘
       │
       ▼
┌─────────────────┐
...
└──────┬──────────┘
       │
       ├── 有 tool_calls ──► 4. 执行工具 ──► 5. 将结果加入消息 ──► 回到 2
       │
       └── 有 content ──► 6. 返回最终答案
                            │
                            ▼
                    7. extract_memories() → 存入长期记忆
```

### 9.2 多 Agent 协作流程

```
用户: "分析项目代码质量并修复关键 bug"
    │
    ▼
┌─────────────────┐
│ Orchestrator     │  决定任务分解和调度顺序
└──────┬──────────┘
       │
       ├── 1. 审计 Agent (可能在 GPU 机器上)
       │     ├── recall() 上次审计的发现
       │     ├── 分析代码，发现 5 个问题
       │     ├── remember() 存入发现
       │     └── 返回结果给 Orchestrator
